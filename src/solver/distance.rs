...
pub fn color_dist(r: f32, g: f32, b: f32) -> [f32; NUM_COLORS] {
    ...
    // Colors 0-7
    let dr = pr - f32x8::from(COLORS_R[0]);
    let dg = pg - f32x8::from(COLORS_G[0]);
    let db = pb - f32x8::from(COLORS_B[0]);
    #[rustfmt::skip]
    ...
    // Colors 8-15
    let dr = pr - f32x8::from(COLORS_R[1]);
    let dg = pg - f32x8::from(COLORS_G[1]);
    let db = pb - f32x8::from(COLORS_B[1]);
    #[rustfmt::skip]
